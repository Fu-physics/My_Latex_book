...
\subsection{Install pacakge in anaconda}

\begin{verbatim}
$which conda
$/Users/fuli/anaconda/bin/pip install pacakge
\end{verbatim}

\subsection{numpy}

\textbf{numpy.loadtxt} loads csv file to an array \footnote{\url{https://docs.scipy.org/doc/numpy/reference/generated/numpy.loadtxt.html}}
...
\textbf{numpy.savetxt} saves an array to a csv text file.\footnote{\url{https://docs.scipy.org/doc/numpy/reference/generated/numpy.savetxt.html}}
...
Transfermation fo np.array 

\begin{tcolorbox} 
	\begin{verbatim}
import numpy as np
np.array([])    # create a blank array
np.array(list)  # Transfer list fo np.array 
np.append(a, b)  #append a np.array with b np.array
	\end{verbatim} 
\end{tcolorbox}

\subsection{pandas}

\textbf{Merge, join, and concatenate}\footnote{\url{https://pandas.pydata.org/pandas-docs/stable/merging.html}}
...
In [5]: result_vertical_append = pd.concat([df1, df2, df3])    

In [6]: result_horizental_append = pd.concat([df1, df4], axis=1, join_axes=[df1.index])
...
\textbf{Create DataFrame}\footnote{\url{http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html}}
